...
const AdminRequests = () => {
    ...
    const [filterCategory, setFilterCategory] = useState('complaint'); 
    ...
    const fetchRequests = async () => {
        setLoading(true);
        try {
            // Fetch from the new unified admin endpoint
    ...
    const handleStatusUpdate = async (id, newStatus, rawType) => {
        try {
            // Context-aware API selection
            const endpoint = rawType === 'Submission' ? `/admin/submissions/${id}` : `/admin/reports/${id}`;
            
            // Map unified status back to model-specific status if needed
            // Reports use: 'open', 'in-progress', 'resolved', 'closed'
            // Submissions use: 'pending' (open), 'verified' (resolved), 'rejected' (closed)
            let apiStatus = newStatus;
            if (rawType === 'Submission') {
                if (newStatus === 'resolved') apiStatus = 'verified';
                if (newStatus === 'closed') apiStatus = 'rejected';
                if (newStatus === 'open') apiStatus = 'pending';
            }

            await api.put(endpoint, { status: apiStatus });
            fetchRequests();
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800">Helpdesk Tickets Management</h2>
                    <div className="flex gap-2 mt-2">
                        <button
                            onClick={() => setFilterCategory('complaint')}
                            className={`px-4 py-1 rounded-full text-xs font-bold transition ${filterCategory === 'complaint' ? 'bg-[#1F3C88] text-white' : 'bg-white text-gray-500 border border-gray-200'}`}
                        >
                            Complaints
                        </button>
                        <button
                            onClick={() => setFilterCategory('cleanup')}
                            className={`px-4 py-1 rounded-full text-xs font-bold transition ${filterCategory === 'cleanup' ? 'bg-[#1F3C88] text-white' : 'bg-white text-gray-500 border border-gray-200'}`}
                        >
                            Cleanups
                        </button>
                    </div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {requests.map((req) => (
                                    <tr 
                                        key={req.id} 
                                        className={`hover:bg-gray-50 transition-colors cursor-pointer ${req.status === 'escalated' ? 'bg-red-50/50' : ''}`} 
                                        onClick={() => setSelectedRequest(req)}
                                    >
                                        <td className="py-4 pl-6">
                                            <span className="font-mono text-sm font-bold text-blue-600">
                                                {req.ticketId || `TKT-${req.id.slice(-6)}`}
                                            </span>
                                            {req.status === 'escalated' && (
                                                <span className="ml-2 inline-block w-2 h-2 rounded-full bg-red-500 animate-ping"></span>
                                            )}
                                        </td>
                                        <td className="py-4">
                        ...
                        >
                            ...
                            <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex flex-wrap justify-end gap-3">
                                {selectedRequest.status !== 'closed' && (
                                    <button
                                        onClick={() => handleStatusUpdate(selectedRequest.id, 'closed', selectedRequest.rawType)}
                                        className="px-4 py-2 rounded-xl font-bold border border-gray-200 text-gray-600 hover:bg-gray-100"
                                    >
                                        {selectedRequest.rawType === 'Submission' ? 'Reject' : 'Close'}
                                    </button>
                                )}
                                {selectedRequest.status === 'open' && selectedRequest.rawType !== 'Submission' && (
                                    <button
                                        onClick={() => handleStatusUpdate(selectedRequest.id, 'in-progress', selectedRequest.rawType)}
                                ...
                                <button
                                    onClick={() => handleStatusUpdate(selectedRequest.id, 'resolved', selectedRequest.rawType)}
                                ...
                                >
                                    ...
                                    {selectedRequest.rawType === 'Submission' ? 'Approve & Credit' : 'Mark Resolved'}
                                </button>
                            </div>
                        </motion.div>
                    </div>
