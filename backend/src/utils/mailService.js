...
exports.sendSubmissionEmail = async (user, submission) => {
  ...
  if (submission.photos && submission.photos.length > 0) {
    submission.photos.forEach((photo, index) => {
      ...
      photoHtml += `<div style="margin-bottom: 10px;">
        <img src="${photo.startsWith('http') ? photo : `${process.env.BASE_URL || 'http://localhost:5000'}${photo}`}" 
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
      <div style="background: linear-gradient(135deg, #0f172a 0%, #063a2e 100%); color: white; padding: 20px; text-align: center;">
        <h1 style="margin: 0; font-size: 24px;">CIVIC सेतु</h1>
        <p style="margin: 5px 0 0; opacity: 0.8;">Action Received</p>
      </div>
      <div style="padding: 30px; color: #333;">
        <h2 style="color: #063a2e;">Hello ${user.name},</h2>
        <p>Thank you for contributing to a cleaner Bharat! We have received your civic action submission.</p>
...
exports.sendStatusUpdateEmail = async (user, submission) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
      <div style="background: ${isApproved ? 'linear-gradient(135deg, #065f46 0%, #059669 100%)' : 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)'}; color: white; padding: 20px; text-align: center;">
        <h1 style="margin: 0; font-size: 24px;">CIVIC सेतु</h1>
